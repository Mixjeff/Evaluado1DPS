...
export default function AutoGrid() {
  ...
  const handleButtonAddClick = () => {
    if (selectedPeli !== null) {
      ...
      if (selectedMovie && !selectedMovies.some(movie => movie.id === selectedPeli)) {
        setSelectedMovies(prevSelectedMovies => [...prevSelectedMovies, selectedMovie]);
      }
    }
  }
  ...
  const handleQuantityChange = (movieId, newQuantity) => {
    const updatedMovies = selectedMovies.map(movie =>
      movie.id === movieId ? { ...movie, cantidadSeleccionada: newQuantity } : movie
    );
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: params => (
        <input type="number" id="quantity" step="1" min="1" max="20"
          defaultValue={cantidadSeleccionada}
          onChange={e => handleQuantityInputChange(params.row.id, e.target.value)}
        ></input>
      ),
      width: 175,
      sortable: false,
  ...
  return (/*Como maximo hay 12 columnas*/
    <>
      <Box sx={{ flexGrow: 1 }}>
        ...
        <Grid container spacing={3}>
          ...
          <Grid xs={12} md={6}>
            <Types texto={"Lista de compras"}></Types>
          </Grid>
        </Grid>
        ...
        <Grid container spacing={4}>
          ...
          <Grid xs={3} md={3}>
            <BasicButtons buttonText={"Agregar"} onClick={handleButtonAddClick}  ></BasicButtons>
          </Grid>
          <Grid xs={1} md={2}></Grid>
        </Grid>
        <Grid container spacing={3}>
